...
**Estado actual** (detalle completo en `docs/mvp-status.md` y `docs/build-log.md`): activo, motor de recomendación personalizado por perfil real de usuario (Fase 1 del plan cerrada), deployeado (Fase 2), identidad visual propia "Hybrid critic notebook" (Fase 3 cerrada, ver `DESIGN.md`), 158 tests de backend. Sesión 2026-07-18: integrado "current picks" (home) y "catalog statistics" reales (footer) inspirados en el prototipo de Lovable, arreglado el mapa de afinidad (roto en prod por `datetime()` de SQLite contra Postgres) y un fix grande de performance (pool de conexiones + paralelización de llamadas a TMDb — login de ~8s a ~2.85s en producción, medido). Falta: que Matías active la cuenta de Resend para que el mail de recuperación funcione en producción.
