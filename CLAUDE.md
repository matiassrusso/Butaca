...
- **Picker "Mi perfil / Letterboxd / A mano" + marco "cómo funciona" + estrellas arrastrables en touch, mergeados** (`2b45626`): era el trabajo de la sesión del 2026-09-02 que había quedado sin commitear en main; Matías lo reconoció ("que el usuario apriete siempre lo mismo, pero que englobe todo") y pidió rebasarlo y mergearlo. Único ajuste: `refine: false` en `/recommend/profile`. Los puntos 2-6 de "aspectos explicativos" de esa sesión también se hicieron (solo texto en `/recommend`, ES/EN: salida "A mano" para quien no tiene Letterboxd, qué es cada grupo del paso 2, cómo leer el % y S/D, que el feedback entrena la próxima tanda, y un "siguiente paso" con links a `/rate` y `/games`). Fuera de alcance, visto de paso: las opciones de género/vibra/movimiento llegan del backend en español aunque la página esté en inglés.
- **Menú lateral sin animación desde el 2026-08-25** (fix de a11y de la auditoría nocturna montaba el panel solo con `open`, y `playOpen()` corría antes de que existiera): arreglado con el panel siempre montado + `inert` (`2789b2e`), verificado en Chrome real forzando `progress()` de los tweens de GSAP.
- Limpieza: 10 worktrees viejos y 16 branches mergeadas borrados.

**Dónde retomar:** que Matías pruebe `/recommend` en butaca.xyz y confirme (picks al toque, why en ~2s). Lo que quedó para él está en la nota, sección "Para Matías": rotar `BUTACA_ADMIN_TOKEN` (lo pasé por query string en un `/admin/stats` y quedó en un access log de Render — error mío), y si encaro los tests que salen a la red real (hallazgo nuevo: `taste_profile.match_titles` pega a TMDb con la key falsa y el 401 se traga; un test hace 82 llamadas HTTPS, por eso la suite oscila 37-100s). Siguiente palanca de perf si el frío molesta: la fase de discover sigue secuencial (~6 `search/person` + ~10 `discover`, ~2-3s).
